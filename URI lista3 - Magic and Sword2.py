...
while casos != 0:
    ...
    if y0 <= (raio + posAtk_Y) <= y0 + altura:
        acerto = True
    if x0 <= (raio + posAtk_X) <= x0 + largura:
        acerto = True
    if y0 <= abs(raio - posAtk_Y) <= y0 + altura:
        acerto = True
    if x0 <= abs(raio - posAtk_X) <= x0 + largura:
        acerto = True
    ...
    while pontoatingido == 0:
        if pontoatingido != 1:
            for ax in range(x0, (x0+largura+1)):
                if pontoatingido == 1:
                    break
                else:
                    for ay in range(y0, (y0+altura+1)): 
                        if pontoatingido == 1:
                            break
                        else:
                            distancia = (((posAtk_X-ax)**2)+((posAtk_Y-ay)**2))**(1/2)
                            #print(distancia)
                            if distancia <= raio:
                                pontoatingido = 1
                                dano = atk
